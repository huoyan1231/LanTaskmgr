...
一小批进程一旦被终止会让 Windows 立即蓝屏或重启（例如 `system`、`csrss`、`wininit`、`smss`、`services`、`lsass`、`winlogon`、`registry`、`memory compression` …），对这些进程的结束请求会被服务器拒绝——它会在 `/list` 里以红色标出，但**单条**受保护的 PID 才会被拒绝，其余进程照常处理。
...
* 密码以明文存储在 `settings.ini` 中，登录时以常量时间比对；验证通过后签发一个随机的 `HttpOnly` 会话令牌（绑定到登录时的 IP 与浏览器指纹，换 IP 即失效）。
* 这里**没有**登录失败封禁 / IP 限流：失败只返回 `401` 并记录日志，并不会封锁该 IP（请勿据此放松网络安全）。
* 切勿通过路由器转发该端口。
